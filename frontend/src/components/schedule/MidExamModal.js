...
function MidExamModal({ onClose, onGenerate }) {
  ...
  const validate = () => {
    ...
    if (step === 2) {
      if (!studyTime) errs.studyTime = 'Select your study preference'
      if (currentProgress < 0 || currentProgress > 100) errs.currentProgress = 'Progress must be between 0 and 100'
    }
        ...
        {step === 3 && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
            ...
            <div style={{ background: '#111', border: '1px solid #1e1e1e', borderRadius: 12, padding: '1rem' }}>
              ...
              <Row label="Current" value={`${currentProgress}%`} />
