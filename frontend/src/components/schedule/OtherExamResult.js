...
function examTip(phase, daysToExam, subject) {
  if (daysToExam <= 1) return `Exam day is tomorrow! Do a light review only — no heavy cramming. Get 8 hours of sleep and stay calm.`
  if (daysToExam <= 3 && phase === 'revise') return `You're in the final stretch for ${subject}. Focus on weak areas and key points — not new content.`
  if (phase === 'learn') return `This is the deep learning phase. Understanding the material now makes revision far easier later.`
  if (phase === 'practice') return `Work through official past papers and model answers under timed conditions — the single best exam prep technique.`
  if (phase === 'revise') return `Compress your notes to a single page. If you can recall the key points from memory, you know it.`
  return `Stay consistent. Even 30 focused minutes each day beats hours of last-minute cramming.`
}

function OtherExamResult({ data, onBack }) {
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '1rem', alignItems: 'flex-start', justifyContent: 'space-between' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            {[
              { label: 'Papers',  value: exams.length },
          ...
          {exams.map((exam) => (
            <div key={exam.subject} style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 6 }}>
                <div style={{ width: 10, height: 10, borderRadius: '50%', background: exam.color, flexShrink: 0 }} />
                <span style={{
